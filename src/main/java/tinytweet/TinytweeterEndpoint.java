...
@Api(name = "tinytweeter", apiKeyRequired = AnnotationBoolean.FALSE)
public class TinytweeterEndpoint {
	...
	public Tweet createTweet(@Named("authorID") Long userID, @Named("authorUsername") String username,
		...
		if (!result.isEmpty()) {
			...
			List<Hashtag> hashtags = ofy().load().type(Hashtag.class).list(); // on recupere une seule fois la liste des
																				// hashtags
			for (String s : result) {
				boolean found = false;
			...
			// Mise a jour du tweet avec les hashtags
